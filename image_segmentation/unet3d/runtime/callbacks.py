...
def get_callbacks(flags, logger, local_rank, world_size):
    ...
    if local_rank == 0:
        if not flags.benchmark:
            callbacks.append(EvaluationCallback(logger, metric="mean_dice"))
...
class EvaluationCallback(BaseCallback):
    def __init__(self, logger, metric):
        self._logger = logger
        self._best_metrics = {}
...
class CheckpointCallback(BaseCallback):
    ...
    def on_epoch_end(self, epoch, metrics, model, optimizer, *args, **kwargs):
        self._last_state = {'last_model_state_dict': model.state_dict(),
        ...
        if metrics[self._main_metric] > self._best_metric:
            ...
            self._best_state = {'best_model_state_dict': model.state_dict(),
